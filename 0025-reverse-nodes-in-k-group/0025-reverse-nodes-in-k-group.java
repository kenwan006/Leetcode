...
* eg: -1(dummy) -> 1 -> 2 ->3 ->4 -> 5 -> 6, k = 3, start = -1, ptr = 1, when count == 3, ptr walks to 4, and reverse nodes between -1 and 4, run reverse(-1, 4) we get new start = 1, -1(dummy) -> 3 -> 2-> 1(new start)-> 4 -> 5 -> 6
*/
class Solution {
    ...
    //eg: 0(start) -> 1(revTail) -> 2 -> 3 -> 4(end), after the reverse 0 -> 3 -> 2 -> 1(revTail) -> 4, return the revTail 
    public ListNode reverse(ListNode start, ListNode end) {
        ...
        start.next = prev; //point 0(start) to 3(new head)
        revTail.next = end; //point 1 to 4
        return revTail;
    }
}